...
$id_usuario =filter_var($obj->usuario);

$sql="call SP_LEER_USUARIOSPORTADORES('$id_usuario');";
$result=mysqli_query($con,$sql);
    while($row = mysqli_fetch_array($result)){
        // Array temporal para crear una sola categoría
        $tmp = array();
        $tmp["up_nombre"] = $row["up_nombre"];
        $tmp["ap_apellidoPat"] = $row["ap_apellidoPat"];
        $tmp["ap_apellidoMat"] = $row["ap_apellidoMat"];
